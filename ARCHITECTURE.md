...
The FFI boundary lives in `Slap.FFI` (CRC-32, Adler-32, BPS diff) and `Slap.Compress` (decompression). Rust allocates output buffers; Haskell copies into `ByteString` and calls `rusty_free`. Adding a new decompressor — for example, lzma for VCDIFF/xdelta3 secondary compression — follows the existing pattern: a function in `compress.rs`, a `pub unsafe extern "C"` wrapper in `lib.rs`, a `foreign import ccall` and a public function in `Slap.Compress`. This layer is expected to grow over time.