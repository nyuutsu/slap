...
A Rust staticlib, fat LTO, `panic=abort`, linked into the Haskell binary via FFI. CRC-32 and Adler-32 (via `crc32fast` and a hand-rolled adler32), SA-IS suffix array, BPS diff, and decompression for zlib, gzip, bzip2, and xdelta3-flavored LZMA (via pure-Rust `flate2`, `bzip2-rs`, and `lzma-rs`, so the staticlib has no C dependencies and Cargo handles cross-platform builds cleanly).

The FFI boundary lives in `Slap.FFI` (CRC-32, Adler-32, BPS diff) and `Slap.Compression.Stream` (decompression). Rust allocates output buffers; Haskell copies into `ByteString` and calls `rusty_free`. Adding a new decompressor follows the existing pattern: a function in `compress.rs` (or its own module when it carries real mechanism, as `lzma.rs` does), a `pub unsafe extern "C"` wrapper in `lib.rs`, a `foreign import ccall` and a public function in `Slap.Compression.Stream`. This layer is expected to grow over time.