...
import Slap.Status (SlapError(..), GetErrorMessage(..), DecompressionFailure(..), BSDiffSection(..), Parsed(..))
...
parseBSDiff (PatchFileContents input)
  ...
  | rawControlSize < 0 || rawDiffSize < 0 || rawTargetSize < 0 = Left (ParseError LabelBSDiff (GetErrorMessage "invalid header (negative size)"))
  | otherwise = do
