...
parseSome patchContents = case detectFormat patchContents of
  ...
  Just (PatchDirect FormatAPSN64)
    ...
    Right SomePatch
      ...
      , patchIsDifferential = False
      , patchApply          = InMemory
            { inMemoryApply = \source -> pure (APSN64.applyAPSN64Memory patch source) }
      , patchUndo           = Nothing
      , patchVerification   = noVerification
  ...
  Just (PatchDiff FormatNINJA2) -> do
    ...
    Right SomePatch
      ...
      , patchIsDifferential = True
      , patchApply          = InMemory
            { inMemoryApply = \source -> pure (NINJA2.applyNINJA2Memory patch source) }
      , patchUndo           = Nothing
      , patchVerification   = noVerification
  ...
  Just (PatchDirect FormatNINJA1) -> do
    ...
    Right SomePatch
      ...
      , patchIsDifferential = False
      , patchApply          = InMemory
            { inMemoryApply = \source -> pure (NINJA1.applyNINJA1Memory patch source) }
      , patchUndo           = Nothing
      , patchVerification   = noVerification
  ...
  Just (PatchDirect FormatPMSR) -> do
    ...
    Right SomePatch
      ...
      , patchIsDifferential = False
      , patchApply          = InMemory
            { inMemoryApply = \source -> pure (PMSR.applyPMSRMemory patch source) }
      , patchUndo           = Nothing
      , patchVerification   = noVerification
      , patchWarnings       = parseWarnings
  ...
  Just (PatchDirect FormatPCHTXT) -> do
    ...
    Right SomePatch
      ...
      , patchIsDifferential = False
      , patchApply          = InMemory
            { inMemoryApply = \source -> pure (PCHTXT.applyPCHTXTMemory patch source) }
      , patchUndo           = Nothing
      , patchVerification   = noVerification
      , patchWarnings       = parseWarnings
...
parseAPSGBABlock patchContents = do
  ...
  Right SomePatch
    ...
    , patchIsDifferential = True
    , patchApply          = InMemory
          { inMemoryApply = \source -> pure (APSGBA.applyAPSGBAMemory patch source) }
    , patchUndo           = Nothing
    , patchVerification   = noVerification
