-- | The VCDIFF address cache: the codec vocabulary both directions share for the compact encoding of COPY addresses (RFC 3284 §5,
-- @docs\/vcdiff\/core\/spec.md@ "Address cache").
--
-- A decoder maintains two small caches, a round-robin /near/ cache and a modulo-slotted /same/ cache,
-- resetting them per window and updating both after every COPY.
-- A later COPY can then name its address as a short distance off a recent one (NEAR) or as a single byte when a slot already holds it (SAME),
-- instead of the full offset (SELF) or a distance back from the write head (HERE). The encoder chooses among those modes;
-- the decoder reads whichever was chosen.
--
-- The cache's home, because the encoder and decoder do not run the same /kind/ of cache but /one/ cache,
-- which must hold byte-identical state at every step: the moment they diverge,
-- a NEAR or SAME address decodes to the wrong place and the patch is silently wrong. So 'recordAddress', the post-COPY update,
-- has exactly one definition, run by 'decodeCopyAddress' on the way in and 'selectCopyAddressMode' on the way out,
-- the two side by side here as inverses, the way 'Slap.VCDIFF.CodeTable' keeps its serializer beside its deserializer.
--
-- The vocabulary is format-defined and pure (no 'Slap.ByteParser', no 'Slap.Status'), so it sits below 'Slap.VCDIFF.Parse'
-- (which imports it and re-exports the testing surface) and 'Slap.VCDIFF.Create' (which imports the selection), neither reaching into the other.
...
-- | @s_near@: the number of near slots a cache holds. A role newtype over 'Int' so it cannot be transposed with 'SameBlockCount':
-- the two share a base type and are built side by side from two adjacent wire bytes
-- ('Slap.VCDIFF.Parse' peels a custom table's @s_near@\/@s_same@ pair),
-- so a bare-'Int' pair would admit a silent swap decoding every COPY address through the wrong cache.
...
-- The default code table fixes these ('defaultAddressCacheConfig'); a custom code table declares its own. The cache carries its configuration
-- so the sizes drive the round-robin wrap, the slot bounds, and the same-block arithmetic at runtime, rather than being baked into the types.
...
-- | The default code table's cache configuration: four near slots and three same blocks, the core's nine address modes (0–8).
-- The one place @4@ and @3@ are named.
...
-- | A near-cache slot index, in @[0, s_near)@. Constructed only within bounds,
-- by 'classifyAddressMode' from a mode in the near band and by 'advanceNearWriteSlot' whose modulus keeps it in range,
-- so a slot the cache lacks is unrepresentable, the guarantee total over any @s_near@ and living where the index is made.
...
-- | A same-cache within-block slot byte: the one-byte SAME operand indexing within a 'SameBlockIndex'. A 'Word8'
-- because a same block holds 256 slots.
newtype SameSlotByte = SameSlotByte Word8
  deriving (Eq, Show)

-- | The two address caches a VCDIFF decoder maintains in lockstep with the encoder, the configuration that sizes them,
-- and the next near slot to overwrite. Both reset to empty at the start of every window and update after every COPY.
--
-- The two caches are one idea twice: each an 'IntMap' keyed by slot, read with a zero default (an untouched slot decodes address 0,
-- so the empty 'IntMap' /is/ the semantics) and written one slot at a time. The cache threads linearly through the decode,
-- each value consumed once to produce the next; a per-COPY 'IntMap' path-copy is the cheap update an array clone would not be.
...
-- | The slots in one same block, fixed by the format: a same-mode COPY's one-byte operand indexes within the block,
-- so a block spans 256 slots regardless of configuration.
...
-- | The first address mode that names a near slot. SELF (0) and HERE (1) precede the near band, so it always begins at mode 2,
-- whatever the cache sizes. A near mode names the slot at its distance past this base, and a near slot's mode byte is this base plus its index:
-- the one mapping 'classifyAddressMode' reads and 'selectCopyAddressMode' writes.
firstNearMode :: Int
firstNearMode = 2

-- | The first address mode that names a same block: the near band fills @s_near@ modes up from 'firstNearMode',
-- so the same band begins just past it. Decode ('classifyAddressMode') and encode ('selectCopyAddressMode') must place this boundary identically,
-- or a same-mode address decodes through the wrong band, so it has one definition here, derived from @s_near@ (RFC 3284 §5.3).
...
-- A mode at or above this names no band: 'classifyAddressMode' returns 'Nothing',
...
-- | What a COPY address mode byte asks for, classified before any bytes are read: one of the two varint modes, a near-slot read,
-- or a same-block read. The wire's mode arithmetic (RFC 3284 §5.3's @2 + s_near + s_same@ bands) is collapsed into the classifier;
-- everything after dispatches on named cases.
...
-- | Name a mode byte's family against a cache configuration: SELF, then HERE, then 'nearSlotCount' near slots,
-- then 'sameBlockCount' same blocks (RFC 3284 §5.3's @2 + s_near + s_same@ bands). 'Nothing' is a mode past the last band,
-- the 'UnknownAddressMode' decline at the caller.
...
-- | Why 'decodeCopyAddress' could not produce an address. Mapped to a 'Slap.Status.VCDIFFMalformation' by the caller,
-- which holds the instruction index these lack.
data AddressDecodeFailure
  = AddressSectionExhausted
  | UnknownAddressMode !Word8
  deriving (Eq, Show)

-- | The product of one COPY-address decode: the absolute address into the superstring, plus the post-state the decode leaves behind,
-- the cache with the address recorded and the address-section cursor advanced past the consumed bytes.
...
-- | Decode one COPY address from the address section, given the cache, the current @here@ position in the superstring,
-- and the address mode (families classified by 'classifyAddressMode'). The upstream half of a two-stage pipeline:
-- the absolute @U@ offset decoded here is what 'Slap.VCDIFF.Apply.resolveCopyAddress' later resolves into a physical read.
decodeCopyAddress
...
-- | The encoder's choice for one COPY: the mode byte to emit (matching a 'Slap.VCDIFF.CodeTable.CopyAddressMode' the active table can name),
-- the operand that mode reads back to the address, and the cache after recording the address.
...
-- | Choose the cheapest encoding of a COPY's absolute superstring address against the current cache and the write head @here@,
-- then record the address so the encoder's cache tracks the decoder's. The inverse of 'decodeCopyAddress', reusing the same slot arithmetic
-- so "is this address in its slot" is the very computation 'recordAddress' placed it by: agreement by reuse, not parallel re-derivation.
--
...
-- SAME is a single byte, so it wins whenever the address already sits in its same-slot and SELF would spend more than one.
-- NEAR is the smallest forward delta off a near slot, HERE the distance back from @here@; each is taken only when strictly shorter than SELF.
-- A mode that merely ties SELF (an untouched near slot whose zero default equals the address,
-- a same-slot holding an address SELF encodes just as short) leaves the plain offset in place, so a cache mode appears exactly
-- where it shrinks the section and SELF is the default everywhere else.
...
selectCopyAddressMode cache here address = recordInto chosen
  where
    ...
    -- SAME: available exactly when the address's same-slot already holds it (an untouched slot holds zero, legitimately matching address zero).
    -- The operand is the byte indexing within the block.
