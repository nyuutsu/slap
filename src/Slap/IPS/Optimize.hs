...
-- i.e. when @G ≤ ipsCopyRecordOverhead − 1@. The looser threshold
-- @G ≤ ipsCopyRecordOverhead@ would also merge in the tie case
-- @G = ipsCopyRecordOverhead@, where merging neither saves nor
-- costs any bytes — strictly equivalent on cost. We keep the
-- strict-cheaper threshold so the optimizer's decisions are
